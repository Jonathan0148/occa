  ...
  public apiUrl: string;
    ...
    constructor(private nav: NavController,
      ...
      private SrBuscaPuntos() {
            this.apiUrl =  this.loading.m_UrlWS + '/getPuntosLlavesRecibirSUP';
      ...
      private SetWS()
     {
         ...
         if(data1["Table"] != null) {
           ...
           if(this.dataAux1[0].RESP=="0")
           {
             ...
             this.presentToast("Precinto Recibido Exitosamente");
             this.getPuntosRonda();
           }
           else{
             this.presentToast("Error: El Precinto NO pudo ser Recibido");
             //this.getPuntosRonda();
           }
         }
         else{
     ...
     private SrInsertaWS() {
       ...
       let headers = new Headers();
       headers.append('Content-Type', 'application/x-www-form-urlencoded');
       let params = 'stRTagID=' + this.tagId + '&inREstado=2' + '&stRValidacion=0' + '&stRUsuario=' + this.loading.m_Apellidos + ' ' +this.loading.m_Nombres +'&stREmpleado=' + this.stLEmpleado +'&stRPuntoID=' + "" +'&stRUsuarioAPP=' + this.loading.m_UsuarioApp +'&stRPasswordAPP='+ this.loading.m_PasswordApp +'&stRCodigoWSAPP='+ this.loading.m_CodigoWSApp +'&stRIMEI='+ this.loading.m_IMEI;
       //let params='';
       return new Promise(resolve => {
         //this.http.post('http://192.168.3.8:1368/icwebmobile/consulta_puestos.php', creds, {headers: headers})
         this.http.post(this.apiUrl,params,{headers: headers}).subscribe(data => {
      ...
      private SrBuscaPuntosBusqueda(stRBusqueda) {
            this.apiUrl =  this.loading.m_UrlWS + '/getPuntosLlavesBusqRecibirSUP';
            console.log(this.apiUrl);
            //this.apiUrl = 'http://192.168.0.5:14757/WSIdentificacion.asmx/Identificacion?stRFoto='+stRFotoAux;
            //let creds = JSON.stringify({stRFoto: this.stRFoto});
            //postData.append('stRFoto' , stRFotoAux);
